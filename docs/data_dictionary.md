...
> all 112 vendor fields stored as native columns (7 TEXT + 105 REAL); no blob since 2026-08-12; `date` = vendor `datekey`
...
| `date` | date (YYYY-MM-DD) | Date Key — [Entity] The Date Key represents the SEC filing date for AR dimensions (ARQ;ART;ARY); and the [REPORTPERIOD] for MR dimensions (MRQ;MRT;MRY). In addition; this is the observation date used for [Price] based data such as [MarketCap]; [Price] and [PE]. |
...
- `metrics` stores 19 of the vendor's 22 METRICS fields: `as_of` is the vendor `date` renamed; `lastupdated`, `ma200w`, `ma50w` are dropped. (Reviewed for the same full-column treatment as SF1 if it ever matters.)

- `sf1` stores **all 112 vendor fields as native columns** (7 TEXT key/date fields + 105 REAL indicators; no blob since the 2026-08-12 migration; `date` = vendor `datekey`).
